...
import type { CoachConfig } from './resolveCoachConfig';
export { coachProvider } from './resolveCoachConfig';
 ...
 *
 *   default (direct)        → Qwen via DashScope, holding the resolved prompt
 ...
 */
export function makeSocialCoachClient(config: CoachConfig): CareyAIClient {
  if (config.metadata.provider === 'aibots') {
  ...
  if (!config.systemPrompt) throw new Error('Direct coach requires a resolved system prompt');
